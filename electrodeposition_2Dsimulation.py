...
st.markdown("""
    This app simulates copper deposition using a phase field model (φ = 1 for electrodeposit, φ = 0 for electrolyte), incorporating Butler-Volmer kinetics, an advection term, and adjustable ε and β. Concentration flux is from the anode towards the interface, with zero concentration in the electrodeposit phase.
""")
...
epsilon = st.sidebar.slider("Interface Width (epsilon, cm)", 0.005, 0.3, 0.008, 0.005, help="Controls interface thickness; higher values stabilize the interface.")
...
t_max = st.sidebar.number_input("Total Time (t_max, s)", 10.0, 2000.0, 500.0, 10.0)
...
z = st.sidebar.number_input("Ion Charge (z)", 1, 30, 8)
F = 96485  # Faraday constant (C/mol)
R = 8.314  # Gas constant (J/(mol·K))
T = 298    # Temperature (K)
k = st.sidebar.number_input("Deposition Rate Constant (k, s^-1)", 0.001, 0.1, 0.09, 0.001)
...
if st.session_state.simulation_results:
    ...
    # Plot phase field (φ), concentration (c), and potential (φ_l)
    st.write("**Phase Field, Concentration, and Electrolyte Potential**")
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=y, y=phi, mode='lines', name='φ (Phase Field)'))
    fig.add_trace(go.Scatter(x=y, y=c, mode='lines', name='c (Concentration)', yaxis='y2'))
    fig.add_trace(go.Scatter(x=y, y=phi_l, mode='lines', name='φ_l (Potential)', yaxis='y3'))
    fig.update_layout(
    ...
    if vis_type != "None":
        if "Heatmap" in vis_type:
            ...
            fig_heatmap = go.Figure(data=go.Heatmap(z=data, x=time_history, y=y, colorscale='Viridis'))
            ...
            fig_surface = go.Figure(data=[go.Surface(z=data, x=time_history, y=y)])
    ...
    # On-the-fly VTS file download
    st.write("**Download VTS File**")
...
1. Adjust parameters in the sidebar:
   - **epsilon**: Increase (e.g., 0.05–0.1 cm) to stabilize the interface.
   - **beta**: Increase (e.g., 2.0–5.0) to stabilize φ at 0, 1.
   - Others: Phi_anode, i0, etc., for deposition rate.
2. Click 'Run Simulation' to start.
3. Use the time slider to view φ (1 = electrodeposit, 0 = electrolyte), c, and φ_l.
4. Select visualizations (heatmaps or 3D surfaces).
5. Download VTS files for the selected time step.
6. Clear temporary files with 'Clear Output Files'.

### Notes
- **φ = 1** (electrodeposit), **φ = 0** (electrolyte).
- Concentration is zero in electrodeposit (φ = 1), with flux from anode (y = Ly) to interface in electrolyte (φ = 0).
